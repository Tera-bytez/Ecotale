    ...
    private static class EcoTopCommand extends AbstractAsyncCommand {
        ...
        protected CompletableFuture<Void> executeAsync(CommandContext ctx) {
            ...
            balances.values().stream()
                ...
                .limit(10)
                .forEach(balance -> {
                    // Try to get cached name, fallback to truncated UUID
                    String displayName;
                    if (h2Storage != null) {
                        String cachedName = h2Storage.getPlayerName(balance.getPlayerUuid());
                        displayName = cachedName != null ? cachedName : balance.getPlayerUuid().toString().substring(0, 8) + "...";
                    } else {
                        displayName = balance.getPlayerUuid().toString().substring(0, 8) + "...";
                    }
                    
                    String formatted = Main.CONFIG.get().format(balance.getBalance());
                    ctx.sendMessage(Message.join(
                        Message.raw("#" + rank[0]++ + " ").color(Color.GRAY),

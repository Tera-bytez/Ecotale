...
public class EcoAdminGui extends InteractiveCustomUIPage<EcoAdminGui.AdminGuiData> {
    ...
    public void handleDataEvent(@NonNullDecl Ref<EntityStore> ref, @NonNullDecl Store<EntityStore> store, 
        ...
        if (data.configName != null) {
            ...
            if (data.configName.length() <= 20) {
                Main.CONFIG.get().setCurrencyName(data.configName);
            }
            refreshUI(ref, store);
            return;
        }
    ...
    private void buildLogTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        // Query H2 with filter and pagination
        int totalCount = h2Storage.countTransactions(logFilter.isEmpty() ? null : logFilter);
        List<TransactionEntry> entries = h2Storage.queryTransactions(
            logFilter.isEmpty() ? null : logFilter, 
            LOG_SIZE, 
            logPage * LOG_SIZE
        );
        ...
        if (totalCount == 0) {
            cmd.set("#LogCountInfo.Text", "No transactions");
        } else {
            cmd.set("#LogCountInfo.Text", 
    ...
    private void buildConfigTab(@NonNullDecl UICommandBuilder cmd) {
        ...
        cmd.set("#CurrencyNameInput.Value", config.getCurrencyName());
